...
from app import db
from models.models import Usuario, Post, Comentario, UserCredential, Categoria
from schemas.schemas import UserRegisterSchema, LoginSchema, UserSchema, PostSchema, ComentarioSchema, CategoriaSchema
...
class PostsAPI(MethodView):
    ...
    def post(self):
        ...
        post = Post(titulo=data["titulo"],contenido=data["contenido"],usuario_id=user_id,categoria_id=data["categoria_id"]) 
        db.session.add(post)
        db.session.commit()
...
class CategoriaAPI(MethodView):
    def get(self):
        ...
        return categoriasSchema.dump(categorias), 200
    ...
    def post(self):
        ...
        data = request.json
        categoria = Categoria(nombre=data["nombre"], descripcion=data.get("descripcion"))
